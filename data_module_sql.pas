...
begin
 case curTDoc of
    ...
    dtPostingResort: Result := 'ќприходование“оваровѕересортица';
    dtDiscardingResort: Result := '—писание“оваровѕересортица';
  end;
end;
...
begin
  ...
  // считываем значени€ в массив
  for i := 0 to (DataSet.Fieldcount-1) do
    aField[i] := DataSet.fields[i].Value ;

  DataSet.Append ;

  // помещаем значени€ массива в новую запись
  for i := 0 to (DataSet.Fieldcount-1) do
...
var
  ...
  s :string;
begin
  if EventStatus = esOK   then  //ѕишем в Log
   begin
...
{BM := ADataSet.GetBookMark;
  ...
  begin
    ...
    IsLookup := (Pos('ID_', ADataSet.FieldDefs[i].Name) <> 0); // дополнительные услови€ отрезаны
    if IsLookup then
    try
      ...
      // определим на что ссылаетс€, у мен€ в проекте это —ущность/Entity
      //LookupEnt := TEntity(EnumValue(EntP, FieldName, 'xe'));
      // спец. менеджер выдаЄт нужный датасет дл€ данной энтити
